...
def make_all_cleans(N, inds):
    sigs = []
    for i in range(N):
        w = 2*np.pi*i/N
        sigs.append([np.exp(1j * w * t) for t in inds])
    return sigs
...
for N in Ns:
    ...
    for _ in range(trials):
        ...
        while not curr_good:
            indices = np.sort(np.random.choice(range(N), size=m, replace=False))
            #cleans = [np.take(full_signals[i], indices) for i in range(N)]
            cleans = make_all_cleans(N, indices)
            #print('made cleans')
